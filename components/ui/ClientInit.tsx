...
const initServicesPeel = () => {
  ...
  const card4  = document.getElementById('svc4') as HTMLElement | null;
  const hintEl = document.getElementById('svcHint') as HTMLElement | null;

  if (!wrapEl || !card1 || !card2 || !card3 || !card4) {
    console.log('Peel animation: elements not found');
    return;
  }

  console.log('Peel animation initialized (4 cards)!');

  // Front-to-back stacking: card1 on top, card4 at bottom
  ...
  /* ── Phase timeline (0 → 1 of scroll through section) ─────
     0.00 → 0.18  Card 1 (Web) PEEL
     0.18 → 0.34  REST  (Card 2/SEO fully visible)
     0.34 → 0.50  Card 2 (SEO) PEEL
     0.50 → 0.66  REST  (Card 3/AI fully visible)
     0.66 → 0.82  Card 3 (AI) PEEL
     0.82 → 1.00  REST + Card 4 (ERP) fully revealed
  ─────────────────────────────────────────────────────────── */
  const C1_END = 0.18;
  const C2_BEG = 0.34;
  const C2_END = 0.50;
  ...
  function updatePeel() {
    ...
    if (progress < C1_END) {
      ...
      card3!.style.transform  = peel(1);
      card3!.style.opacity    = '0';
      card3!.style.visibility = 'hidden';
    }

    /* ── CARD 4 (ERP) — last card, just rises and stays ───── */
    if (progress < C1_END) {
      const s = 0.88 + e1 * 0.04;
      card4!.style.transform  = `perspective(1800px) translateY(${60 * (1 - e1)}px) scale(${s})`;
      card4!.style.opacity    = String(0.4 + e1 * 0.3);
      card4!.style.visibility = 'visible';
    } else if (progress < C2_BEG) {
      card4!.style.transform  = 'perspective(1800px) translateY(40px) scale(0.92)';
      card4!.style.opacity    = '0.7';
      card4!.style.visibility = 'visible';
    } else if (progress < C2_END) {
      const e2 = ease(clamp((progress - C2_BEG) / (C2_END - C2_BEG)));
      const s  = 0.92 + e2 * 0.04;
      card4!.style.transform  = `perspective(1800px) translateY(${40 * (1 - e2)}px) scale(${s})`;
      card4!.style.opacity    = String(0.7 + e2 * 0.3);
      card4!.style.visibility = 'visible';
    } else if (progress < C3_BEG) {
      card4!.style.transform  = 'perspective(1800px) translateY(20px) scale(0.96)';
      card4!.style.opacity    = '1';
      card4!.style.visibility = 'visible';
    } else if (progress < C3_END) {
      const e3 = ease(clamp((progress - C3_BEG) / (C3_END - C3_BEG)));
      const s  = 0.96 + e3 * 0.04;
      card4!.style.transform  = `perspective(1800px) translateY(${20 * (1 - e3)}px) scale(${s})`;
      card4!.style.opacity    = '1';
      card4!.style.visibility = 'visible';
    } else {
      // Final card stays flat and fully visible
      card4!.style.transform  = flat;
      card4!.style.opacity    = '1';
      card4!.style.visibility = 'visible';
    }
  }
